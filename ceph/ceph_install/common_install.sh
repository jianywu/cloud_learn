...
# 如果22.04是jammy，而20.04是focal，通过lsb_release -sc查看。
...
(
cat << EOF
$ip ceph-deploy.example.local ceph-deploy
$ip ceph-mon1.example.local ceph-mon1
$ip ceph-mon1.example.local ceph-mon2
$ip ceph-mon1.example.local ceph-mon3
$ip ceph-mgr1.example.local ceph-mgr1
$ip ceph-mgr1.example.local ceph-mgr2
$ip ceph-node1.example.local ceph-node1
$ip ceph-node2.example.local ceph-node2
$ip ceph-node2.example.local ceph-node3
EOF
) >> /etc/hosts
...
# 如果上面的运行有问题，可以编译安装最新版本的ceph-deploy
...
ssh-copy-id cephadmin@ceph-server
"
